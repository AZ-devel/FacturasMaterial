...
import { insertFacturaSchema, insertLineaFacturaSchema, type Cliente, type Producto } from "@shared/schema";
...
export default function FacturaForm({ numeroFactura }: FacturaFormProps) {
  ...
  const calcularSubtotal = () => {
    return form.getValues("lineas").reduce((sum, linea) => sum + linea.total, 0);
  };
  ...
  const handleProductoSelect = (producto: Producto, lineaIndex: number) => {
    ...
    form.setValue(`lineas.${lineaIndex}.precio`, parseFloat(producto.precio));
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            {selectedCliente ? (
              <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                <div>
                  <h4 className="font-medium text-gray-900">{selectedCliente.nombre}</h4>
                  <p className="text-sm text-gray-600">
                    {selectedCliente.email && `${selectedCliente.email} • `}
                    {selectedCliente.nif && `NIF: ${selectedCliente.nif}`}
                  </p>
                </div>
                <Button 
                  type="button" 
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="date" {...field} className="input-material" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button type="button" onClick={agregarLinea} size="sm" className="btn-primary">
                    ...
                    <TableRow key={field.id}>
                              ...
                              <div className="space-y-2 max-h-96 overflow-y-auto">
                                {productos?.map((producto: Producto) => (
                                  <Card 
                                    ...
                                    onClick={() => handleProductoSelect(producto, index)}
                                  >
                                    <CardContent className="p-3">
                                      <div className="flex justify-between">
                                        ...
                                        <div className="text-right">
                                          <p className="font-medium">{formatCurrency(parseFloat(producto.precio))}</p>
                                          {producto.categoria && (
                      ...
                      <TableCell>
                        <div className="font-medium">
                          {formatCurrency(form.watch(`lineas.${index}.total`) || 0)}
                        </div>
                      </TableCell>
                      <TableCell>
                        {fields.length > 1 && (
        ...
        <div className="flex justify-end space-x-4 py-6">
          <Button type="button" variant="outline" onClick={() => setLocation("/facturas")}>
            Cancelar
          </Button>
          <Button 
            type="submit" 
